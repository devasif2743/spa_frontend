...
export default function POS() {
  ...
  const submitOrder = async (orderPayload) => {
    try {
      const data = await addBilling(orderPayload);
  ...
  const confirmPayment = () => {
    ...
    const billedStaffObj = allstaffList.find(
              ...
              <div className="mt-2 space-y-1">
                ...
                {memberships.map((m) => (
                  ...
                  >
                    ...
                    <span>
                      {m.plan_name} – exp{" "}
                      {new Date(m.expires_on).toLocaleDateString()} (
                ...
                >
                  ...
                  {staffList.map((s) => (
                    <option key={s.id} value={s.id}>
                      {s.name}{" "}
                      {s.next_free_in > 0 &&
                        `– next free in ${Math.ceil(s.next_free_in / 60)} min`}
                    </option>
                  ))}
                </select>
              </label>
