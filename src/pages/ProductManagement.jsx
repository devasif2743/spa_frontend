...
const ProductManagement = () => {
  ...
  const loadProducts = async (page = 1, search = "") => {
    setLoading(true);
    try {
      const prodRes = await fetchProducts(page, 6, search);
      console.log("ssss",prodRes)
      if (prodRes.status) {
        // Laravel paginate response
        setProducts(prodRes.products);
        setPagination({
          current_page: prodRes.pagination.current_page,
          last_page: prodRes.pagination.last_page,
          total: prodRes.pagination.total,
        });
      }
    } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.status) {
        loadProducts(pagination.current_page, searchTerm);
        handleCloseDrawer();
      } else {
