...
export default function POS() {
  ...
  const loadProducts = async (page = 1, search = "") => {
    const res = await fetchProducts(page, 12, search); // server-side search
    if (res?.status) {
  ...
  const submitOrder = async (orderPayload) => {
    try {
      const data = await todayBilling(orderPayload);
      if (data.status === true) {
        alert("✅ Appointment stored successfully!");
        resetAll();
        loadProducts(1, searchTerm); // reload after order
      } else {
  ...
  const confirmPayment = () => {
    ...
    const billedStaffObj = staffList.find((s) => s.id === Number(billedById));
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {pagination.last_page > 1 && (
            <div className="flex justify-center items-center space-x-2 mt-6">
              <button
                ...
                onClick={() =>
                  loadProducts(pagination.current_page - 1, searchTerm)
                }
        ...
        {/* ===== Cart, Voucher, Membership, Totals, Checkout ===== */}
        {/* keep your cart + totals + modal UI here */}
