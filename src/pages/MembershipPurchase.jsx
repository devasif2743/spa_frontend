...
export default function MembershipManagement() {
  ...
  // const [drawerType, setDrawerType] = useState<"add" | "view" | null>(null);
  const [drawerType, setDrawerType] = useState(null); // "add" | "view"
  ...
  const openTransactionDrawer = async (membership) => {
    setSelectedMembership(membership);
    setDrawerType("view");
    setDrawerOpen(true);
    setLoadingTx(true);
    try {
      const res = await membershiphistorydetails(membership.id);
  ...
  return (
    <div className="p-6">
          ...
          <tbody>
            {currentRecords.map((m) => (
              <tr key={m.id} className="hover:bg-gray-50 text-center">
                ...
                <td className="px-2 py-1 border">₹{parseFloat(m.amount).toFixed(2)}</td>
                <td className="px-2 py-1 border">{m.payment_method}</td>
                <td className="px-2 py-1 border">
                  {m.expiration ? new Date(m.expiration).toLocaleDateString() : "N/A"}
                </td>
                <td className="px-2 py-1 border">
                  <button
                    onClick={() => openTransactionDrawer(m)}
      ...
      {/* Right Drawer */}
      <div
        className={`fixed inset-0 z-50 flex transition ${drawerOpen ? "visible" : "invisible"}`}
      >
        {/* Overlay */}
        <div
          ...
          {drawerType === "view" && (
            <>
              <h2 className="text-xl font-bold mb-4">
                Transactions – {selectedMembership?.customer_name}
              </h2>
              {loadingTx ? (
                  ...
                  <tbody>
                    {transactions.map((t) => (
                      <tr key={t.id} className="hover:bg-gray-50 text-center">
                        ...
                        <td className="p-2 border">{new Date(t.start_time).toLocaleString()}</td>
                        <td className="p-2 border">{new Date(t.end_time).toLocaleString()}</td>
